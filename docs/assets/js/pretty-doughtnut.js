	...
	render: function(o) {
		for (var value in o) {
			if (!doughnutWidget.charts[value + 'Chart']) {
				...
				label.append(perc);
				if (value.localeCompare("Swiss") == 0){
					label.append('<div class="labelText">' + "Swiss<br>German" + '</div>');
				} else if (value.localeCompare("Cpp") == 0){
					label.append('<div class="labelText">' + "C++" + '</div>');
				}
				else{
